...
describe("getPlanModelStrategy", () => {
  it("routes Free directly through fast OpenAI by default", () => {
    ...
    for (const stage of [strategy.blueprint, strategy.detail, strategy.repair]) {
      ...
      expect(stage.model).toBe("gpt-4.1-mini");
  ...
  it("uses the paid reasoning defaults for legacy Pro", () => {
    ...
    expect(strategy.blueprint).toMatchObject({ timeoutMs: 60_000, maxOutputTokens: 6_000 });
