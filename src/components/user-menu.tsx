      ...
      <div ref={modalRef} className="account-center" role="dialog" aria-modal="true" aria-labelledby="account-center-title">
        ...
        <main className="account-content">
          ...
          {tab === "plans" ? <section>
            ...
            <div className="plan-summary-strip"><div><span>Gói đang dùng</span><strong>{planLabel(activePlan)}</strong></div><div><span>Tín dụng hiện có</span><strong>{subscription.credits.total}</strong></div></div>
            ...
            <div className="payment-zone payment-zone-single"><div className={`payment-config ${!canTopup ? 'disabled-zone' : ''}`}><p className="account-section-label">Mua thêm tín dụng</p><div className="flex gap-2"><input id="topup-amount" className="input-field" type="number" min={25000} step={1000} value={topupAmount} disabled={!canTopup} onChange={(e) => setTopupAmount(Number(e.target.value))} /><button id="buy-topup" className="btn-primary shrink-0" disabled={!canTopup || Boolean(busy)} onClick={() => checkout("topup")}>Mua {Math.max(0, topupAmount / 1000)} tín dụng</button></div><p className="mt-2 text-xs text-slate-500">{ownedPaidCard ? `Cộng vào gói ${ownedPaidCard.name}; tín dụng hết hạn cùng gói.` : "Tối thiểu 25.000đ; cần sở hữu gói Trả phí còn hạn."}</p></div></div>
            {payment ? <button type="button" className={`active-payment-strip status-${payment.status}`} onClick={() => setPaymentModalOpen(true)}><span><b>{payment.purchaseType === 'package' ? `Gói ${planLabel(payment.targetPlan)}` : `${payment.credits} tín dụng`}</b><small>{payment.safeReason}</small></span><strong>{paymentStatusLabel(payment.status)} →</strong></button> : null}
          </section> : null}
